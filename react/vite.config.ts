...
const REACT_ROOT = __dirname // react包根目录
const REACT_SRC = resolve(REACT_ROOT, 'src') // react源码目录
...
export default defineConfig(({ mode }) => ({
  ...
  // 构建核心配置 - 库模式打包
  build: {
    // 产物输出目录 - react打包后所有文件输出到包根目录的dist下
    outDir: resolve(REACT_ROOT, 'dist'),
    ...
    lib: {
      entry: resolve(REACT_ROOT, 'index.ts'), // 库入口文件
      name: 'MyMiniReactReact', // 全局变量名（预留UMD/IIFE格式使用）
      fileName: format => `react.${format}.js`, // 产物命名：按模块格式区分
      formats: ['es', 'cjs'], // 输出模块格式：ESModule + CommonJS，适配多环境
    },
    // Rollup底层配置 - 定制库模式打包规则
    rollupOptions: {
      external: [], // 空数组表示无外部依赖，强制将shared源码合并打入react产物
  ...
  plugins: [
    ...
    dts({
      outDir: resolve(REACT_ROOT, 'dist'), // 类型文件输出目录，与JS产物同目录
      entryRoot: REACT_ROOT, // 类型声明的根目录
      include: [REACT_ROOT], // 包含react包的所有文件，确保类型检查覆盖整个包
      tsconfigPath: resolve(REACT_ROOT, 'tsconfig.json'), // 指定当前包的TS配置文件
